...
Write-Host ""
Write-Host "  下一步（直接复制完整一行执行）：" -ForegroundColor Gray
Write-Host "    Gateway：  cd $repoRoot; cargo run -p orcha-gateway $runFlag" -ForegroundColor Gray
if (-not $SkipTs) {
    Write-Host "    Adapter：  cd $repoRoot; pushd packages\orcha-feishu-adapter; node dist\main.js; popd" -ForegroundColor Gray
}
